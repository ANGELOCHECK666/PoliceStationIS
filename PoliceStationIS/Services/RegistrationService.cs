    ...
    public class RegistrationService
    {
        public static void Register(RegistrationData data)
        {
            using (var connection = DatabaseConnection.GetConnection())
            {
                ...
                // Сотрудник и его учётная запись должны создаваться вместе.
                // Если одна операция завершится ошибкой, изменения откатываются.
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        int sexId = GetSexId(connection, data.Gender);
                        int departmentId =
                            GetDepartmentId(connection, data.Department);
                        int postId = GetPostId(connection, data.Position);
                        int rankId = GetRankId(connection, data.Rank);
                        int passportIssuanceId =
                            GetPassportIssuanceId(connection, data.IssuedBy);
                        ...
                        InsertUser(connection, employeeId, data);
            ...
            string sexName)
        {
            ...
            {
                command.Parameters.AddWithValue("@name", sexName);
            ...
            string postName)
        {
            ...
            using (var command = new NpgsqlCommand(query, connection))
            {
                command.Parameters.AddWithValue("@name", postName);
            ...
            RegistrationData data)
        {
            ...
            using (var command = new NpgsqlCommand(query, connection))
            {
                ...
                // При регистрации используется существующая роль,
                // заданная текущей логикой проекта.
                command.Parameters.AddWithValue(
                    "@user_role_id",
                    11);
